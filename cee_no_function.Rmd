...
```{r read_csvs}
processed <- read_csv("data/raw_data/gonio_aux_merge.csv")
processed
cee <- read_csv(file = "data/raw_data/ATLANTIC-BRS_CEE Metadata.csv") %>%
  mutate(date_time = as.POSIXct(paste(Date, as.character(CEE.Times)), format = "%m/%d/%y %H:%M:%S"))
cee
series <- read_csv("data/raw_data/All_Series.csv") %>%
...
Larry's chunk draft
```{r}
cee19_01_start <- cee$date_time[3]
cee19_01_end <- cee$date_time[4]
...
# total2 <- merge(cee19_01, cee19_01_counts, by= "DeployID", all=TRUE)
total2 <- merge(cee19_01_counts, cee19_01_bef, by= "DeployID", all=TRUE)
total2 <- merge(total2, cee19_01_after, by= "DeployID", all=TRUE) %>% 
  replace(is.na(.), 0)
...
final_df <- final_df %>%
  filter(start_date < cee19_01_end,
         end_date > cee19_01_start)
