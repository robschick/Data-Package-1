...
```{r locations}
locations <- read_csv("/cloud/project/data/raw_data/processed_locations.csv")

locations <- locations %>%
  mutate(Source = rep("locations",nrow(locations))) %>%
...
aux <- raw_aux %>%
  mutate(date_time = as.POSIXct(paste(Date, as.character(Time)), tz = "GMT", format = "%m/%d/%y %H:%M:%S"),
...
processed_gonio_aux_locs <- processed_gonio_aux_locs %>%
  filter(date_time != "2019-05-27 15:05:28") %>%
  filter(date_time != "2019-05-27 15:06:31") %>%
  filter(date_time != "2019-05-27 15:07:36") %>%
  filter(date_time != "2019-06-02 14:21:46")
```
